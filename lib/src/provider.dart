...
class _ProviderPlusElement extends InheritedElement {
  ...
  final _hooks = ReversedMap<Element, ReversedMap<int, HookState>>();
  final _index = <Element, int>{};

  void _disposeOlds(BuildContext context) {
    final index = _index[context] ?? 0;
    final hooks = _hooks[context] ?? ReversedMap<int, HookState>();
  ...
  void _disposeContextData(BuildContext context) {
    // _typedRefs.forEach((key, map) => map.remove(context)); is it needed to remove typedRefs?
...
extension ContextAPI on BuildContext {
  ...
  T use<T>(Hook<T> hook) => _ProviderPlus.of(this)!.use(this, hook);
  ...
  T findHookValueByType<T>({Object? key}) {
    final state = findHookStateByType<T, Hook<T>>(key: key);

    return state.build(this);
  }

  HookState<R, Hook<R>> findHookStateByType<R, T extends Hook<R>>(
      {Object? key}) {
    final element = getElementForInheritedWidgetOfExactType<_ProviderPlus>()
        as _ProviderPlusElement?;

    if (element == null) {
      throw Exception('ProviderPlus not found for $T.');
    }

    for (var hooks in element._hooks.values) {
      for (var state in hooks.values) {
        if (state.hook is T && (key == null || state.hook.key == key)) {
          return state as HookState<R, Hook<R>>;
        }
      }
    }

    throw Exception('HookState not found for $T.');
  }
  ...
  TickerProvider get vsync => useSingleTickerProvider();

  TickerProvider useSingleTickerProvider({Object? key}) {
    return use(_SingleTickerProviderHook(key: key));
  }

  T useProvider<T>(Create<T> create, {Object? key}) {
    return use(ProviderHook(create: create, key: key));
  }
...
class ReversedMap<K, V> extends MapBase<K, V> {
...
class ProviderHook<T> extends Hook<T> {
  const ProviderHook({required this.create, super.key});
  final Create<T> create;
...
class _ProviderHookState<T> extends HookState<T, ProviderHook<T>> {
  late final value = hook.create(context);
